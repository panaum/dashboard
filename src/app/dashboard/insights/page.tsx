...
export default async function InsightsPage() {
  ...
  const issueCount = (p: (typeof pages)[number]) => p.issues.length;
  ...
  const platforms = [...platMap.entries()]
    .map(([platform, e]) => ({ platform, ...e, avg: e.pages ? e.issues / e.pages : 0 }))
    .filter((p) => p.pages >= 3)
    .sort((a, b) => b.avg - a.avg);
  const maxPlatAvg = Math.max(1, ...platforms.map((p) => p.avg));

  // By developer
  const devMap = new Map<string, { name: string; built: number; issues: number; rep: number }>();
  for (const p of pages) {
