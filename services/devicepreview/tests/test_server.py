...
def _load(env: dict[str, str]):
    ...
    for k in ("RUNS_DIR", "RETAIN_RUNS", "RUN_TIMEOUT_S", "DEVICEPREVIEW_KEY", "MAX_RUNNING", "DEVICEPREVIEW_CONCURRENCY"):
...
class Service(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.mod = _load({"RUNS_DIR": str(cls.runs), "DEVICEPREVIEW_KEY": KEY, "RETAIN_RUNS": "2",
    ...
    def test_unconfigured_service_refuses_everything(self):
        ...
        type(self).mod = _load({"RUNS_DIR": str(self.runs), "DEVICEPREVIEW_KEY": KEY, "RETAIN_RUNS": "2",
                                "DEVICEPREVIEW_CONCURRENCY": "1"})
    ...
    def test_start_poll_fetch_and_retention(self):
        ...
        self.assertFalse((self.runs / ids[0]).exists(), "the oldest run was pruned (RETAIN_RUNS=2)")
