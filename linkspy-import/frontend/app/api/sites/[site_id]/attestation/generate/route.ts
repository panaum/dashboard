...
export async function POST(req: NextRequest, { params }: { params: Promise<{ site_id: string }> }) {
  ...
  try {
    const res = await fetch(`${backend()}/api/sites/${site_id}/attestation/generate`, { method: "POST", cache: "no-store", body, headers: { ...(await backendAuthHeaders()), "Content-Type": "application/json" } });
