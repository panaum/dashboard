...
export async function POST(req: NextRequest, { params }: { params: Promise<{ site_id: string }> }) {
  ...
  try {
    const res = await fetch(`${backend()}/api/sites/${site_id}/inbound-404/preview`, {
      method: "POST", cache: "no-store", body, headers: { ...(await backendAuthHeaders()), "Content-Type": "text/csv" } });
