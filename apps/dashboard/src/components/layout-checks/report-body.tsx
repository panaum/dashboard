...
function Group({ group, shotUrl, viewportOf }: { group: ReportGroup; shotUrl: (profileId: string) => string; viewportOf: (profileId: string) => number }) {
  ...
  return (
    <div className="flex flex-col gap-4 border-t border-border-soft py-6 first:border-t-0 first:pt-2 @3xl:flex-row @3xl:gap-6">
      {group.sample ? (
        <EvidenceCrop
          src={shotUrl(group.sample.profileId)}
...
export type ReportRun = { id: string; checkedAt: Date; report: DpReport } | null;

/**
 * The report itself: one run, written for whoever owns the site. The team page
 * and the shared page both render this; they differ only in the frame around
 * it and in where the pictures come from — the team's session-guarded route,
 * or the share token's own.
 */
export function ReportBody({ site, run, shotUrl, backHref, openHref, children }: {
  site: { url: string; label: string | null };
  run: ReportRun;
  /** Where a device's fold is served from, for the evidence pictures. */
  shotUrl: (profileId: string) => string;
  /** The team page's way back; the shared page has none. */
  backHref?: string;
  /** The team page's way into the full check; the shared page has none. */
  openHref?: string;
  /** Anything the frame wants beside the title — the share control, say. */
    ...
    <div className="@container flex flex-col gap-8 pb-16">
      ...
      <article className="mx-auto w-full max-w-[64rem] rounded-2xl border border-border-soft bg-card px-8 py-10 shadow-xs @3xl:px-14 @3xl:py-14 print:border-0 print:shadow-none">
        <header className="flex flex-wrap items-center justify-between gap-3 border-b border-border-soft pb-6">
          ...
          <span className="flex flex-wrap items-center gap-3 text-[13px] text-text-secondary">
            {when ? `Layout check · ${when}` : "Layout check"}
            {children}
          </span>
        </header>
          ...
          <>
            ...
            {blocking.length > 0 && (
              <section className="mt-10">
                ...
                <div className="mt-2">
                  {blocking.map((g) => <Group key={g.rule} group={g} shotUrl={shotUrl} viewportOf={viewportOf} />)}
                </div>
              </section>
            )}
            ...
            {next.length > 0 && (
              <section className="mt-10">
                ...
                <div className="mt-2">
                  {next.map((g) => <Group key={g.rule} group={g} shotUrl={shotUrl} viewportOf={viewportOf} />)}
                </div>
              </section>
            )}
            ...
            <footer className="mt-10 flex flex-wrap items-center justify-between gap-3 border-t border-border-soft pt-6 text-[12px] text-text-secondary">
              <span className="max-w-[70ch]">Every finding above is measured on a real rendering of this page, not eyeballed.{openHref ? " The full evidence, with screenshots per device, is in the Dashboard." : ""}</span>
              {openHref && <Link href={openHref} className="font-medium text-accent print:hidden">Open the full check →</Link>}
            </footer>
          </>
        )}
      </article>
