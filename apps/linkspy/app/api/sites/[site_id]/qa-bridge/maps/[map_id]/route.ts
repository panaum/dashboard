...
export async function DELETE(_req: NextRequest, { params }: { params: Promise<{ site_id: string; map_id: string }> }) {
  const { site_id, map_id } = await params;
  try {
    const res = await fetch(`${backend()}/api/sites/${site_id}/qa-bridge/maps/${map_id}`, {
