...
export async function POST(req: NextRequest, { params }: { params: Promise<{ site_id: string; contract_key: string }> }) {
  const { site_id, contract_key } = await params;
  const body = await req.text();
  try {
    const res = await fetch(`${backend()}/api/sites/${site_id}/contracts/${contract_key}/confirm`, {
