| `SLACK_BOT_TOKEN` | Slack bot token (`xoxb-…`) with `chat:write` + `im:write`. When set, a board @mention, a reply and a due-date reminder are sent as a **direct message** to that person (`chat.postMessage`, member id as the channel). This is the right shape for these: they are personal. A DM that fails falls back to the webhook and says so | secret | — | No — unset ⇒ the webhook is used |
| `SLACK_WEBHOOK_URL` | Incoming webhook for board @mention pings (`lib/slack.ts`). Same mechanism as LinkSpy's notifiers. **A webhook is bound to the one conversation it was created for** — a `<@U…>` only notifies someone who is already in it, and if the webhook belongs to a person's own DM, nobody else ever sees the message (this happened: every board mention landed in one person's self-DM). Prefer `SLACK_BOT_TOKEN`. Either way, members need `TeamMember.slackUserId` (Team page → edit) | secret | *(same name on Railway linkspy-api, set separately)* | No — unset ⇒ mentions are stored with `notifiedAt` null and the comment reports "not notified" |
